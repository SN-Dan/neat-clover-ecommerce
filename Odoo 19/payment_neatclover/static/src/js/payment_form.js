...
patch(PaymentForm.prototype, {
    ...
    /**
     * Prepare the inline form for direct payment.
     *
     * @override method from @payment/interactions/payment_form
     * @private
    ...
    /**
     * Process Clover checkout via iframe or redirect.
     *
     * @override method from @payment/interactions/payment_form
     * @private
     * @param {string} providerCode - The code of the selected payment option's provider.
    ...
    async _processDirectFlow(providerCode, paymentOptionId, paymentMethodCode, processingValues) {
        ...
        const useIframe = [true, 'true', 1, '1', 'True', 'TRUE'].includes(
            processingValues.neatclover_use_iframe
        );
        if (!checkoutUrl) {
            this._enableButton();
    ...
    /**
     * Process Clover token flow via iframe or redirect.
     *
     * @override method from @payment/interactions/payment_form
     * @private
     * @param {string} providerCode - The code of the selected payment option's provider.
    ...
    async _processTokenFlow(providerCode, paymentOptionId, paymentMethodCode, processingValues) {
        if (providerCode !== 'neatclover') {
            await super._processTokenFlow(...arguments);
            return;
        }
