function showTermsPopup(e) {
    ...
    if (!window.isTermsPopupActivated) {
        const closeTerms = document.querySelector(".neatclover_close_terms");
        if (closeTerms) {
            closeTerms.addEventListener("click", function (ev) {
                ev.preventDefault();
...
// Expose for inline onclick handlers on the backend payment provider form.
window.showTermsPopup = showTermsPopup;
window.showActivationForm = showActivationForm;